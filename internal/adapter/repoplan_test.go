...
func makeRepoPlanDir(t *testing.T, repoPath, planName, content string) {
	t.Helper()
...
func TestRepoPlan_LoadAll_ValidPlans(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()

	makeRepoPlanDir(t, dir, "alpha", `# Alpha Plan

- [x] Task 1
- [x] Task 2
- [ ] Task 3
`)
	makeRepoPlanDir(t, dir, "beta", `# Beta Plan

- [ ] Task A
- [ ] Task B
`)

	loader := NewRepoPlanLoader()
...
func TestRepoPlan_LoadAll_MissingDir(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
...
func TestRepoPlan_LoadAll_EmptyTasksMd(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()

	makeRepoPlanDir(t, dir, "empty-plan", "# No tasks here\n\nJust notes.\n")
...
func TestRepoPlan_LoadAll_MixedTasks(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()

	makeRepoPlanDir(t, dir, "mixed", `# Mixed
...
func TestRepoPlan_LoadAll_IndentedTasks(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()

	makeRepoPlanDir(t, dir, "indented", `# Indented
...
func TestRepoPlan_LoadSummary_Aggregation(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()

	makeRepoPlanDir(t, dir, "plan-a", `- [x] Done
- [ ] Pending
`)
	makeRepoPlanDir(t, dir, "plan-b", `- [x] Done 1
- [x] Done 2
- [ ] Pending
`)

	loader := NewRepoPlanLoader()
	summary, err := loader.LoadSummary(dir, "my-repo")
	if err != nil {
